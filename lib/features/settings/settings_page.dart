...
class _SettingsPageState extends ConsumerState<SettingsPage> {
  ...
  void _addEmail(String value) {
    ...
    if (_emails.contains(email)) {
      toastWarn(context, t(context, 'Already added', '已添加'));
      return;
    }
    setState(() {
  ...
  Future<void> _addPush() async {
    const templates = <(String, String)>[
      ...
      ('Teams', 'teams://[GROUP]@[TENANT]/[ALT_ID]/[GROUP_ID]'),
      ('Custom', ''),
    ];
    final template = await showDialog<String>(
      context: context,
      builder: (ctx) => SimpleDialog(
        title: Text(t(context, 'Choose a template', '选择模板')),
        children: [
  ...
  Future<void> _testPush(String url) async {
    try {
      ...
      if (mounted) toastSuccess(context, t(context, 'Test sent', '测试已发送'));
    } catch (e) {
  ...
  Future<void> _saveDisplay() async {
    ...
    try {
      ...
      if (mounted) toastError(context, t(context, 'Failed to save', '保存失败'));
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(t(context, 'Settings', '设置'))),
      body: SafeArea(
        child: ListView(
    ...
    return MCard(
      child: Column(
        ...
        children: [
          ...
          Text(
            t(context,
                'Get notified when a server goes offline or recovers.',
                '服务器离线或恢复时接收通知。'),
            style: TextStyle(
                fontSize: 12, color: theme.colorScheme.onSurfaceVariant),
          ),
          const SizedBox(height: 14),
          Text(t(context, 'Email targets', '邮件目标'),
              style:
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 10),
              Text(t(context, 'Display Settings', '显示设置'),
                  style: const TextStyle(
                      fontSize: 15, fontWeight: FontWeight.w600)),
            ],
          ),
          const SizedBox(height: 14),
          _label(t(context, 'Monitor layout', '监控页布局')),
          SegmentedButton<String>(
            selected: {_display.monitorLayout},
            segments: [
              ButtonSegment(
                  value: 'grid-3', label: Text(t(context, 'Grid 3', '3 列'))),
              ButtonSegment(
                  value: 'grid-2', label: Text(t(context, 'Grid 2', '2 列'))),
              ButtonSegment(
                  value: 'list', label: Text(t(context, 'List', '列表'))),
            ],
          ...
          const SizedBox(height: 14),
          _label(t(context, 'Aggregation', '聚合方式')),
          SegmentedButton<String>(
            selected: {_display.monitorMode},
            segments: [
              ButtonSegment(
                  value: 'avg', label: Text(t(context, 'Avg', '平均'))),
              ButtonSegment(
                  value: 'max', label: Text(t(context, 'Max', '最大'))),
              ButtonSegment(
                  value: 'raw', label: Text(t(context, 'Raw', '原始'))),
            ],
            onSelectionChanged: (s) =>
                setState(() => _display.monitorMode = s.first),
          ),
          const SizedBox(height: 14),
          _label(t(context, 'Y-axis', 'Y 轴范围')),
          SegmentedButton<String>(
          ...
          const SizedBox(height: 14),
          _label(t(context, 'Dashboard layout', '仪表盘布局')),
          SegmentedButton<String>(
            selected: {_display.dashboardLayout},
            segments: [
              ButtonSegment(
                  value: 'grid', label: Text(t(context, 'Grid', '网格'))),
              ButtonSegment(
                  value: 'list', label: Text(t(context, 'List', '列表'))),
            ],
            onSelectionChanged: (s) =>
